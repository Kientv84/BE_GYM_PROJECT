...
const {
  checkBeforeCreate,
  checkBeforeUpdate,
} = require("../ProductController/ProductValidate");
const System_Product = require("../../models/Product/ProductModel");
...
module.exports.update = async function (req, res) {
  ...
  if (validation.success != Ck_ConstantCommon.CK_RESULTS.SUCCESS) {
    ...
    if (checking != Ck_ConstantCommon.CK_RESULTS.SUCCESS) {
      ...
      try {
        const updatedItem = await System_Product.findOneAndUpdate(
          query,
          newValues,
          { new: true }
        );
...
module.exports.delete = async function (req, res) {
  ...
  ///// check before delete when product is using

  if (!dataModel) {
    return res.status(400).send({
      success: Ck_ConstantCommon.CK_RESULTS.ERROR,
      message: "Please chose product to delete",
    });
  }
  System_Product.deleteMany({ _id: dataModel }, function (err, result) {
    if (err) {
      return res.status(400).send({
        success: Ck_ConstantCommon.CK_RESULTS.ERROR,
